...
(function() {
    ...
    let selectedEngine = 'stockfish'; // Default engine selection
...
chessBot.fen = board.game.getFEN()
                ...
                if(board.game.getTurn() ==  board.game.getPlayingAs())
                {
                    ...
                    if(checkfen !== fen)
                    {
                        ...
                        checkfen = fen;
                        const data = await fetch(`https://api-chess-python.onrender.com/api/v1/evaluation`, {
                            method: 'POST',
                ...
                if(!len && can_interval &&hint){
                 ...
                 try{
                    ...
                    console.log("nice");
                     const data = await fetch(`https://api-chess-python.onrender.com/api/v1/best-move`, {
                         method: "POST",
                         headers: {
              ...
              catch(e)
              {
                  console.log("an error has cocured" + e);can_interval=true
                    ...
                    if(fen !== checkfen)
                    {
                        ...
                        cp = resp.evaluation?.cp || 0;
                            $('.myanalysis').remove()
                            if(show_evaluation){
                        const img_moves = resp.winning_chances > 70 ? '🔥' : resp.winning_chances > 50 ? '👍' : '👎';
                 ...
                 <div style="position:absolute;top:0;width:20px;height:20px;font-size:16px;">${img_moves}</div>
                 </div>`)
                            }
...
">
  ...
  <section style="display: flex; flex-direction: column; gap: 6px;">
    <p style="font-size: 20px; font-weight: 600; letter-spacing: 0.04em;">Chess Engine Selection</p>
    <select id="engineSelect" style="padding: 8px; border-radius: 4px; background: #333; color: white; border: 1px solid #555;">
      <option value="stockfish">Stockfish (~3200 ELO)</option>
      <option value="ensemble">Multi-Engine Consensus</option>
...
<input type="color" id="colorPicker"  name="color-changer" value="#000000">
  ...
  <div style="background: linear-gradient(45deg, #00ff00, #00cc00); color: black; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold; margin: 15px 0;">
    ✅ POWERED BY YOUR OWN API<br>
    <small>api-chess-python.onrender.com</small>
  </div>
</div>
`)
